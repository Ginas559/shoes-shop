...
public class VendorShopServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			Shop shop = em.createQuery("SELECT s FROM Shop s WHERE s.vendor = :vendor", Shop.class)
					.setParameter("vendor", vendorRef).getSingleResult();
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
	...
	private String uploadToCloudinary(Part part) throws IOException {
		...
		try (InputStream is = part.getInputStream()) {
			...
			Map<?, ?> res = cloud.uploader().upload(data, ObjectUtils.asMap("folder", "shoes-shop/products", // giữ
																												// nguyên
																												// folder
																												// bạn
																												// đang
