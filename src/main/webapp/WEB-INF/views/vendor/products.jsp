...
<div class="main-products py-4">
  <div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <c:if test="${not empty shop}">
      <span class="text-dark fw-bold">Shop: <strong><c:out value="${shop.shopName}"/></strong></span>
    </c:if>
  </div>
  ...
  <div class="card kpi-card mb-3">
    <div class="card-body">
      ...
      <form method="post" enctype="multipart/form-data" action="<c:url value='${actionPath}'/>" class="row g-3">
        <c:if test="${not empty p}">
          <input type="hidden" name="productId" value="${p.productId}"/>
        </c:if>

        <div class="col-md-4">
          <label class="form-label" for="prodName">Tên</label>
          <input id="prodName" name="name" class="form-control" required value="${not empty p ? p.productName : ''}"/>
        </div>

        <div class="col-md-2">
          <label class="form-label" for="prodPrice">Giá</label>
          <input id="prodPrice" name="price" type="number" step="0.01" min="0.01" class="form-control" required value="${not empty p ? p.price : ''}"/>
        </div>
        ...
        <div class="col-md-2">
          ...
          <c:choose>
            <c:when test="${empty p}">
              <input id="prodStock" name="stock" type="number" class="form-control" value="0" readonly/>
            </c:when>
            <c:otherwise>
              <input id="prodStock" name="stock" type="number" min="0" class="form-control" required value="${p.stock}"/>
            </c:otherwise>
          </c:choose>
        </div>

        <div class="col-md-4">
          <label class="form-label" for="prodCat">Danh mục</label>
          <select id="prodCat" name="categoryId" class="form-select" required>
        ...
        <div class="col-md-6">
          ...
          <input id="prodImg" type="file" name="image" accept=".jpg,.jpeg,.png,.webp,image/*" class="form-control"/>
          <small class="text-muted d-block mt-1">Chọn 1 ảnh (≤ 2MB) — sẽ lấy làm thumbnail.</small>

          <c:if test="${not empty thumbEditing}">
            <div class="mt-2">
              <img src="${thumbEditing}" width="120" class="rounded shadow-sm" alt="Ảnh hiện tại"/>
            </div>
          </c:if>
        </div>
        ...
        <c:if test="${not empty p}">
          <div class="col-md-3">
            <label class="form-label" for="prodStatus">Trạng thái</label>
  ...
  <!-- Tìm kiếm & Lọc (bản mở rộng, có Xóa lọc) -->
  <div class="card kpi-card mb-4">
    <div class="card-body">
      <h5 class="card-title">Tìm kiếm & Lọc</h5>

      <form method="get" action="<c:url value='/vendor/products'/>" class="row g-2" id="vendorFilterForm">
        <!-- Tên SP -->
        <div class="col-12 col-md-4">
          <input class="form-control" name="q" placeholder="Tìm theo tên..." value="${param.q != null ? param.q : (q != null ? q : '')}"/>
        </div>
        ...
        <div class="col-6 col-md-3">
          <select class="form-select" name="categoryId">
            ...
            <c:forEach var="c" items="${categories}">
              <option value="${c.categoryId}" ${ (param.categoryId != null ? param.categoryId : categoryId) == c.categoryId ? 'selected' : ''}>
                <c:out value="${c.categoryName}"/>
              </option>
            </c:forEach>
          </select>
        ...
        <div class="col-6 col-md-2">
          <select class="form-select" name="status">
            <c:set var="stVal" value="${param.status != null ? param.status : status}"/>
            <option value=""           ${empty stVal ? 'selected' : ''}>-- Tất cả --</option>
        ...
        <!-- Size / trang -->
        <div class="col-6 col-md-2">
          <c:set var="sz" value="${param.size != null ? param.size : (size != null ? size : 10)}"/>
        ...
        <div class="col-12 col-md-5">
          <div class="input-group">
            ...
            <input class="form-control" type="number" min="0" name="minPrice" value="${param.minPrice}" placeholder="Từ">
            <span class="input-group-text">→</span>
            <input class="form-control" type="number" min="0" name="maxPrice" value="${param.maxPrice}" placeholder="Đến">
          </div>
        </div>
        ...
        <div class="col-6 col-md-2">
          <select class="form-select" name="sort">
            ...
            <option value=""                 ${empty sortVal ? 'selected' : ''}>Sắp xếp</option>
            <option value="new_desc"         ${sortVal == 'new_desc'     ? 'selected' : ''}>Mới nhất</option>
            <option value="price_asc"        ${sortVal == 'price_asc'    ? 'selected' : ''}>Giá ↑</option>
            <option value="price_desc"       ${sortVal == 'price_desc'   ? 'selected' : ''}>Giá ↓</option>
            <option value="rating_desc"      ${sortVal == 'rating_desc'  ? 'selected' : ''}>Đánh giá ↓</option>
          </select>
        </div>

        <!-- Thuộc tính mở rộng -->
        <div class="col-6 col-md-2">
          <input class="form-control" name="brand"   value="${param.brand}"   placeholder="Brand">
        </div>
        <div class="col-6 col-md-2">
        ...
        <div class="col-6 col-md-2">
          <input class="form-control" name="style"    value="${param.style}"    placeholder="Style">
        </div>
        ...
        <div class="col-12 col-md-auto d-flex gap-2">
          ...
          <!-- XÓA LỌC: quay về /vendor/products, clear toàn bộ query -->
          <a class="btn btn-outline-secondary" href="<c:url value='/vendor/products'/>">Xóa lọc</a>
        </div>
      </form>
    </div>
  </div>
              ...
              <div class="card h-100 vendor-card">
                ...
                <c:choose>
                  <c:when test="${not empty thumbnails[it.productId]}">
                    <img src="${thumbnails[it.productId]}" class="card-img-top product-card-img cover" alt="Ảnh sản phẩm"/>
                  </c:when>
                  <c:otherwise>
                    <img src="${pageContext.request.contextPath}/assets/img/placeholder.png" class="card-img-top product-card-img cover" alt="Placeholder"/>
                  </c:otherwise>
                </c:choose>

                <div class="card-body">
                  <h6 class="fw-bold product-card-title" title="${it.productName}">
                    <c:out value="${it.productName}"/>
                  </h6>

                  <div class="small text-muted">
                    <c:out value="${empty it.category ? '—' : it.category.categoryName}"/>
                  </div>
                  ...
                  <div class="d-flex justify-content-between mt-2 align-items-center">
                    ...
                    <span class="badge ${stocks[it.productId] > 0 ? 'bg-success-subtle text-success-emphasis' : 'bg-danger-subtle text-danger-emphasis'}">
                      Tồn: <c:out value="${stocks[it.productId]}"/>
                    </span>
                  </div>

                  <span class="badge status-badge bg-${it.status == 'ACTIVE' ? 'success' : 'secondary'} mt-2">
                ...
                <div class="card-footer product-actions-footer">
                  <div class="product-actions-grid">
                    ...
                    <a class="btn btn-sm btn-outline-warning" href="${variantsUrl}">Biến thể</a>
                    <a class="btn btn-sm btn-outline-secondary" href="${attrUrl}">Thuộc tính</a>
...
<!-- Vá CSS để đảm bảo nút clickable -->
<style>
...
.vendor-card .product-actions-footer,
.vendor-card .product-actions-footer .product-actions-grid,
.vendor-card .card-footer a,
.vendor-card .card-footer button {
  position: relative;
  z-index: 3;
  pointer-events: auto;
}
