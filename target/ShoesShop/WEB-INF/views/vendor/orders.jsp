...
<main class="container py-4">
  <h2 class="mb-3">Đơn hàng của shop</h2>

  <!-- Filter trạng thái -->
  <form class="row g-2 mb-3" method="get" action="<c:url value='/vendor/orders'/>">
    <div class="col-auto">
      ...
      <select name="status" class="form-select">
        ...
        <c:forEach var="st" items="${fn:split(statuses, ',')}">
          <option value="${st}"
            <c:if test="${st eq status}">selected="selected"</c:if>>
            ${st}
          </option>
        </c:forEach>
      </select>
        ...
        <table class="table table-striped align-middle">
          ...
          <c:forEach var="o" items="${orders}">
            <tr>
              ...
              <td>${o.createdAt}</td>
              <td>
                <form method="post" action="<c:url value='/vendor/orders'/>" class="d-flex gap-2">
                  <input type="hidden" name="orderId" value="${o.orderId}"/>
                  <select name="newStatus" class="form-select form-select-sm">
                    <c:forEach var="st" items="${fn:split(statuses, ',')}">
                      <option value="${st}"
                        <c:if test="${st eq o.status.name()}">selected="selected"</c:if>>
                        ${st}
                      </option>
                    </c:forEach>
                  </select>
