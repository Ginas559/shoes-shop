...
<main class="container py-4">
  <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
    ...
    <form class="row g-2" method="get" action="${ctx}/vendor/dashboard">
      <div class="col-auto">
        <input type="date" class="form-control" name="from" value="${filterFrom}" placeholder="From (YYYY-MM-DD)"/>
      </div>
      <div class="col-auto">
        <input type="date" class="form-control" name="to" value="${filterTo}" placeholder="To (YYYY-MM-DD)"/>
      </div>
      <div class="col-auto">
        <input type="number" class="form-control" name="year" value="${filterYear}" placeholder="Year (YYYY)" min="2000" max="2100"/>
      </div>
      <div class="col-auto">
  ...
  <div class="mb-4 d-flex flex-wrap gap-3">
    ...
    <c:if test="${not empty shop and not empty shop.shopId}">
      <a href="${ctx}/chat?shopId=${shop.shopId}" class="btn btn-outline-primary">💬 Chat với cửa hàng</a>
    </c:if>
  </div>
          ...
          <ol class="mb-0 small">
            <c:forEach var="row" items="${stats.topProducts}">
              <li>
                ...
                — SL: <strong><c:out value="${row[1]}"/></strong>
              </li>
            </c:forEach>
        ...
        <table class="table table-sm align-middle">
          ...
          <c:forEach var="o" items="${recentOrders}">
            <tr>
              ...
              <td><c:out value="${o.user.firstname}"/> <c:out value="${o.user.lastname}"/></td>
              <td><c:out value="${o.totalAmount}"/></td>
              <td><span class="badge bg-secondary"><c:out value="${o.status}"/></span></td>
